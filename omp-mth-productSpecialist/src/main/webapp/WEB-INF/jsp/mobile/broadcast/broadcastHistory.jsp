	...
	<div class="jsbb-bj">
		<header>
			<div class="head">
				...
				<label class="title">${param.type}</label>
				<a class="top-right"></a>
			</div>
		</header><!-- /header -->
		<c:if test="${param.type eq '2016.11.11'}">
			<div class="kaiqi">
...
<script type="text/javascript">
	$(function(){
		...
		//加载抽奖红包数量
		<c:if test="${param.type eq '2016.11.11'}">
			M.post('loadGiftLuckyHistoryTotal?type=${param.type}&token=${token}',{}, function(result){
		...
		//加载今日订单数据
		M.post('loadHistoryOrderTotal?type=${param.type}&token=${token}',{}, function(result){
			var data = result.data || {};
		...
		//加载参与会员统计
		M.post('loadHistoryOrderMemberNum?type=${param.type}&token=${token}',{}, function(result){
		...
		//加载商品分类
		M.post('loadHistoryGoodsCateoryStatistics?type=${param.type}&token=${token}',{}, function(result){
			var data = result.data || {};
		...
		//加载订单流量图
		M.post('loadHistoryOrderTraffic?type=${param.type}&token=${token}',{}, function(result){
			var array = result.data || [];
			var timeArray = new Array();
			var totalArray = new Array();
			for(var i = 0;i < array.length;i++){
				var data = array[i];
				timeArray.push(data.start_time);
	...
	function loadOrderFlowChart(timeArray, totalArray){
		orderFlowChart.setOption({
		    ...
		    xAxis: {
		        ...
		        axisLabel: {
		        	show:false,
		        	interval:${param.type eq '2016.11.11' ? 47 : 17},
		            textStyle: {
		                color: '#ffffff'
		            }
		        },
